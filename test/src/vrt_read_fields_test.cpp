...
TEST_F(ReadFieldsTest, StreamId3) {
    ...
    buf_[0]        = 0xFEFEBEBE;
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 0, &f_), VRT_ERR);
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 1, &f_), 1);
    SCOPED_TRACE(::testing::UnitTest::GetInstance()->current_test_info()->name());
    assert_fields(f_, {{"stream_id", static_cast<uint32_t>(0xFEFEBEBE)}});
}
...
TEST_F(ReadFieldsTest, FractionalSecondsTimestamp) {
    ...
    buf_[0] = 0xBABBABBA;
    buf_[1] = 0xCEECCEEC;
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 1, &f_), VRT_ERR);
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 2, &f_), 2);
    SCOPED_TRACE(::testing::UnitTest::GetInstance()->current_test_info()->name());
...
TEST_F(ReadFieldsTest, EveryOther2) {
    ...
    buf_[2]         = 0xCECEBEBE;
    buf_[3]         = 0xBABBA011;
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 3, &f_), VRT_ERR);
...
TEST_F(ReadFieldsTest, All) {
    ...
    buf_[4]         = 0xADDABEBB;
    buf_[5]         = 0xEFEFEF01;
    ASSERT_EQ(vrt_read_fields(&h_, buf_.data(), 5, &f_), VRT_ERR);
